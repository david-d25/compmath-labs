...
import ru.david.compmath4.gui.modal.NodeCoordinatesModal;
import ru.david.compmath4.math.expression.LagrangeInterpolationExpression;
...
public class LagrangeInterpolationViewer extends InteractiveGraphViewer {
    ...
    static {
        BufferedImage image;
        try {
            image = ImageIO.read(Thread.currentThread().getClass().getResourceAsStream("/delete.png"));
        } catch (Exception e) {
            image = null;
    ...
    private boolean isInterpolationLimited = true;
    ...
    public void setInterpolatedGraph(GraphData interpolationGraph, List<XY> nodes, boolean useOwnInterpolationLimit) {
        clearData();
        actualGraph = null;

        this.interpolationNodes = nodes;
        this.interpolationGraph = interpolationGraph;
        if (useOwnInterpolationLimit) {
            interpolationGraph.setLimitEnabled(isInterpolationLimited);
        } else {
            isInterpolationLimited = interpolationGraph.isLimitEnabled();
        }
        addData(interpolationGraph);
    }
    ...
    public void mouseDragged(MouseEvent e) {
        if (!enableEditing || draggingNode == null || interpolationGraph == null)
