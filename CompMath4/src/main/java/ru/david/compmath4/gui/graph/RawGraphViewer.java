...
public class RawGraphViewer extends JComponent {
    ...
    private static final double MIN_GAP_SIZE = 2;
    ...
    private double lowerY = -10;
    private double upperY = 10;
    ...
    private void drawGraph(GraphData graph, Graphics2D g, int graphIndex) {
        ...
        for (
                float projX = -1;
                projX < getWidth();
                projX += usePrev && hiResRender ? Math.max(0.01, Math.min(1, 1/Math.abs(yDerivative))) : 1
        ) {
