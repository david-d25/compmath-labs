...
import static ru.david.compmath3.math.expression.ArithmeticExpression.Operation.*;
...
public class ExpressionParser {
    ...
    private static Expression parseAddSubtract(String rawExpression, Map<String, Expression> variables) {
        ...
        if (lastAddSymbol == -1 && lastSubtractSymbol == -1)
            return parseMod(rawExpression, variables);
    ...
    private static Expression parseMod(String rawExpression, Map<String, Expression> variables) {
        int lastModSymbol = lastSymbolIgnoringParenthesis(rawExpression, '%');
        if (lastModSymbol == -1)
            return parseMultiplyDivide(rawExpression, variables);

        String leftRaw = rawExpression.substring(0, lastModSymbol);
        String rightRaw = rawExpression.substring(lastModSymbol+1);

        Expression left = parse(leftRaw, variables);
        Expression right = parse(rightRaw, variables);

        return new ArithmeticExpression(left, right, MOD);
    }
    ...
    private static Expression parseFunction(String rawExpression, Map<String, Expression> variables) {
        ...
        if (rawExpression.startsWith("sin"))
            ...
            function = Math::cos;
        else if (rawExpression.startsWith("sec"))
            function = (x) -> 1/Math.cos(x);
        else if (rawExpression.startsWith("csc"))
            function = (x) -> 1/Math.sin(x);
        else if (rawExpression.startsWith("cot"))
            function = (x) -> 1/Math.tan(x);
        else if (rawExpression.startsWith("abs"))
            function = Math::abs;
        else if (rawExpression.startsWith("log"))
            function = Math::log;
        else if (rawExpression.startsWith("tan"))
            function = Math::tan;
        else if (rawExpression.startsWith("tg")) {
            function = Math::tan;
            functionNameLength = 2;
        } else if (rawExpression.startsWith("sqrt")) {
            function = Math::sqrt;
            functionNameLength = 4;
        } else if (rawExpression.startsWith("atan")) {
            function = Math::atan;
            functionNameLength = 4;
        } else if (rawExpression.startsWith("asin")) {
            function = Math::asin;
            functionNameLength = 4;
        } else if (rawExpression.startsWith("acos")) {
            function = Math::acos;
            functionNameLength = 4;
        } else if (rawExpression.startsWith("floor")) {
            function = Math::floor;
            functionNameLength = 5;
        } else if (rawExpression.startsWith("ceil")) {
            function = Math::ceil;
            functionNameLength = 4;
        }
